...
class HomeMemberBloc extends Bloc<HomeMemberEvent, HomeMemberState> {
  ...
  HomeMemberBloc({required this.userRepository}) : super(HomeLoading()) {
    on<HomeGetUserEvent>(_onHomeGetUserEvent);
    on<HomeGetUserOverviewEvent>(_onHomeGetUserOverviewEvent);
  }

  void _onHomeGetUserEvent(
      HomeGetUserEvent event, Emitter<HomeMemberState> emit) async {}

  Future<void> _onHomeGetUserOverviewEvent(
      HomeGetUserOverviewEvent event, Emitter<HomeMemberState> emit) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      var userResult = await userRepository.getUserMember(event.memberId);
      ...
      var result = await userRepository.getOverviewMember(date, event.memberId);
